Pepecoin Core version 0.14.2 is now available from:

  <https://download.pepecoin.org/pepecoin-0.14.2.0/>
...
Pepecoin Core now supports P2SH addresses beginning with M on mainnet and Q on testnet.
...
This only affects users that have explicitly enabled UPnP through the GUI setting or through the -upnp option, as since the last UPnP vulnerability (in Pepecoin Core 0.10.4) it has been disabled by default.
...
- http://testnet.pepecointools.com
- http://testnet.thrasher.io

Developers who require the new testnet blockchain paramaters can find them [here](https://github.com/pepecoin-project/pepecoin/blob/master/src/chainparams.cpp#L220).
...
Pepecoin Core has supported automatically pruning the blockchain since 0.13.2. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/pepecoin-project/pepecoin/blob/master/doc/zmq.md) for
...
Commands sent over the JSON-RPC interface and through the `pepecoin-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`pepecoin-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/pepecoin-cli -named help command="help"
    src/pepecoin-cli -named getblockhash height=0
    src/pepecoin-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/pepecoin-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
The Alert System was disabled and deprecated in Pepecoin Core 0.10.4 and removed in 0.13.2. 
...
here, originally written for Bitcoin but can also be used for Pepecoin:
