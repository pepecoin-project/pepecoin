Pepecoin Core version *0.15.1* is now available from:

  <https://download.pepecoin.org/pepecoin-0.15.1/>
...
A number of changes to the way Pepecoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 frogs per kB (0.00001 PEPE). This is to prepare for dropping the minimum transaction fee to 0.1 frogs per kB (0.0001 PEPE) in the next release.
