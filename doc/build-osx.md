...
1. Clone the Pepecoin Core source code:
    ```shell
    git clone https://github.com/pepecoin-project/pepecoin
    cd pepecoin
    ```

2.  Build Pepecoin Core:

    Configure and build the headless Pepecoin Core binaries as well as the GUI (if Qt is found).
...
## Disable-wallet mode
When the intention is to run only a P2P node without a wallet, Pepecoin Core may be
...
```shell
mkdir -p "/Users/${USER}/Library/Application Support/Pepecoin"

touch "/Users/${USER}/Library/Application Support/Pepecoin/pepecoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Pepecoin/pepecoin.conf"
```

The first time you run pepecoind, it will start downloading the blockchain. This process could
