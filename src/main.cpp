...
uint256 hashGenesisBlock("0xe46211ea0713e5909ffd07f37936196640c5f5cdfa8ad51357a7731e503f22d8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Pepecoin: starting difficulty is 1 / 2^12
...
{
    // Pepecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Pepecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Pepecoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Pepecoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Pepecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Pepecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x592425e10734b0cff9a6a8ff69f1f1cf864c253de3b9619dd4143c25b52b7067");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Reuters 19/Aug/2022 Dollar driven to five-week high by Fed rate hike forecasts";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0490e82005b476aa4e4d919354fd397772d7722bbf51e56fb2716f2553121af561f9f4185b3993cec87cd8c4dc3bdab96d3c82a2f2ddf185f91462f3fa14c25635") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1660922603;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084865389;

        if (fTestNet)
        {
            block.nTime    = 1660922587;
            block.nNonce   = 386060683;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xce726d75ec571f0576eb9095cb401f7a28c23ffeed38afab48f4c04f42f5978c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc9, 0xb7, 0xde }; // Pepecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PepecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PepecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PepecoinMiner(CWallet *pwallet)
{
    printf("PepecoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running PepecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PepecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PepecoinMiner, pwallet));
}
